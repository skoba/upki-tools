...
UPKIからサーバ証明書一括ダウンロードで取得したserverAll.tsvを登録して更新や失活処理のためのTSVファイルを自動生成させることができるようになります。動かせばだいたいわかると思い亜ms宇。
...
- **秘密鍵の保護**は最重要事項です。このシステムでは秘密鍵を生成してダウンロードできるようにはしますが保存はしません。各自で管理してください。
